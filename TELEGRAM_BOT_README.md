...
- `/stop` - Остановить бота (только для администраторов)
...
# Тестирование завершения бота
npm run test-bot-shutdown
...
1. **Через сигналы системы:**
   - `Ctrl+C` (SIGINT)
   - `kill` команда (SIGTERM)

2. **Через команду в Telegram:**
   - `/stop` (только для администраторов)
...
5. Проверьте корректность завершения процесса
